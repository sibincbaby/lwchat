...
async function cmdFind(issueId, json) {
  ...
  if (aliases.length === 0) {
    if (json) {
      out({ ok: false, error: `Issue #${issueId} not found in any configured space` }, true);
    } else {
      console.error(`Issue #${issueId} not found in any configured space`);
    }
    process.exit(1);
  }
...
async function cmdRead(issueId, spaceAlias, json) {
  ...
  if (aliases.length === 0) {
    if (json) out({ ok: false, error: `Issue #${issueId} not found in any configured space` }, true);
    else console.error(`Issue #${issueId} not found in any configured space`);
    process.exit(1);
  }
  ...
  if (spaceAlias) {
    if (!locations[spaceAlias]) {
      if (json) out({ ok: false, error: `Issue #${issueId} not found in space '${spaceAlias}'`, available: aliases }, true);
      else console.error(`Issue #${issueId} not found in space '${spaceAlias}'. Found in: ${aliases.join(", ")}`);
      process.exit(1);
    }
...
async function cmdReply(issueId, message, spaceAlias, json) {
  ...
  if (spaceAlias) {
    ...
    const msg = `Issue #${issueId} is in ${aliases.length} spaces (${aliases.join(", ")}). Specify --space <alias> to choose which thread to reply to.`;
    if (json) out({ ok: false, error: msg, available: aliases }, true);
...
async function cmdPost(spaceArg, message, threadName, json) {
  ...
  if (!resolved) {
    if (json) out({ ok: false, error: `Unknown space: '${spaceArg}'. Pass a configured alias or a raw spaces/... id.` }, true);
    else console.error(`Unknown space: '${spaceArg}'. Run \`lwchat spaces\` for configured aliases.`);
    process.exit(1);
  }
...
async function cmdDm(userArg, message, json) {
  ...
  try {
    ...
    if (json) out({ ok: false, error: e.message }, true);
    else console.error(e.message);
    process.exit(1);
  }
  if (!userId) {
    const msg = `Could not resolve '${userArg}' to a Chat user. Use an email (sibin@linways.com) or a users/<id>, or run \`lwchat members\` to see known names.`;
    if (json) out({ ok: false, error: msg }, true);
    else console.error(msg);
    process.exit(1);
  }
  ...
  if (!space) {
    const msg = `No existing DM space with ${userId}. Open a DM with them in Google Chat once, then retry — lwchat doesn't have permission to create new DM spaces (would need the chat.memberships scope).`;
    if (json) out({ ok: false, error: msg, user_id: userId }, true);
    else console.error(msg);
    process.exit(1);
  }
...
async function cmdSearch(term, { spaceAlias, spaceList, limit = 30, caseSensitive = false }, json) {
  if (!term) {
    if (json) out({ ok: false, error: "search term required" }, true);
    else console.error("Usage: lwchat search <term> [--space <alias> | --spaces a,b,c] [--limit N]");
    process.exit(1);
  }
  ...
  if (spaceAlias) {
    aliases = [spaceAlias];
  } else if (spaceList && spaceList.length) {
    aliases = spaceList;
  } else {
    aliases = config.default_spaces.length > 0 ? config.default_spaces : Object.keys(config.spaces);
  }
...
const DEFAULT_CACHE_TTL_SECONDS = 300; // 5 min
...
async function scanLocations(issueId, config) {
  const aliases = config.default_spaces.length > 0
    ? config.default_spaces
    : Object.keys(config.spaces);
...
const MEMBERS_CACHE_TTL = 24 * 60 * 60 * 1000; // 24 hours

async function getMemberMap(spaceId) {
  const membersData = await loadMembers();
  const cached = membersData.spaces?.[spaceId];

  if (cached && cached.updated_at && (Date.now() - new Date(cached.updated_at).getTime()) < MEMBERS_CACHE_TTL) {
...
async function cmdRestore(name, json) {
  if (!name) {
    ...
    if (backups.length === 0) {
      if (json) {
        out({ ok: false, error: "No backups found" }, true);
      } else {
        console.error("No backups found. Create one with: lwchat backup");
      }
      process.exit(1);
    }
    name = backups[0].name;
